...
--A3 = filter A2 by (date >= 0) and (date < $m) and (domain_name matches '.*icook.*');
...
--  generate group as key, BagToString(EE.domain_name, ',') as value;
  generate CONCAT($0, CONCAT('_',(chararray)'$stime')) as key, BagToString(EE.domain_with_count, ',') as value;
};
describe E2;
